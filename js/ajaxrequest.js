    ...
    $("#stuemail").on("keypress blur", function(){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(data != 0){
                    $("#statusMsg2").html(
                        '<small style="color:red;">Email ID Already Exists!</small>'
                    );
                    $("#signup").attr("disabled", true);
                } else if (data == 0 && reg.test(stuemail)){
                    $("#statusMsg2").html(
                        '<small style="color:green;">Valid Email ID!</small>'
                    );
...
function addStu(){
    ...
    if(stuname.trim() == ""){
        ...
        $("#statusMsg3").html('<small style="color:red;">Please Enter Passward !</small>');
        $("#stupass").focus();
        return false;
    } else {
        $.ajax({
            ...
            success:function(data){
                ...
                if(data == "OK") {
                    $("#successMsg").html(
                        "<span class='alert alert-success'>Registration Succesful !</span>"
                        );
                    clearStuRegField();
                } else if(data == "Failed") {
                    $("#successMsg").html(
